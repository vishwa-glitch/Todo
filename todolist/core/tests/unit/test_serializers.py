...
class TodoSerializerTests(TestCase):
    ...
    def test_create_todo_missing_required_fields(self):
        ...
        self.assertEqual(
            response.data["title"][0], "This field is required."
        )  # Updated message
    ...
    def test_create_todo_due_date_in_past(self):
        ...
        self.assertEqual(
            response.data["due_date"][0], "Due date cannot be in the past."
        )

    def test_create_todo_with_too_many_tags(self):
        tags = [
            {"name": "tag" + str(i)} for i in range(6)
        ]  # 6 tags, one more than the max
        data = {
            "title": "Test Todo",
            "tags": tags,
        }
